    ...
    { 
        Foreach ($F In $File)
        {
            If (Test-Path -Literalpath $F)
            {
                ...
                Write-ToString "$F already exists, updating file to today's lastwritetime"
            }
            Else
            {
                # Create New File. 
                ...
                Write-ToString $Null | Out-File -Encoding Ascii -Literalpath $F
                Write-ToString "$F does not exist, creating an empty file"
                # Alternative
