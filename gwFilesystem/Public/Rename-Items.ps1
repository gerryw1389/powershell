...
#>
    ...
    Process
    {
        If ([Bool]($MyInvocation.BoundParameters.Keys -match 'Recurse'))
        {
            ...
            Write-ToString "Renamed all items in $Source and subfolders from $OldExtension to $NewExtension"
        }
        Else
        {
            Get-Childitem $Source -Filter ( "*" + $OldExtension ) | Rename-Item -Newname { [Io.Path]::Changeextension($_.Name, $NewExtension) }
            Write-ToString "Renamed all items in $Source from $OldExtension to $NewExtension"
        }
    }
