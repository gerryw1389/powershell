...
<#######<Script>#######>
<#######<Header>#######>
# Name: Compare-DisksToVMDisks
<#######</Header>#######>
<#######<Body>#######>
Function Compare-DisksToVMDisks
{
    <#
.Synopsis
This function is supposed to compare VMWare disks to physical disks. Never used so I don't know if it works or not.
.Description
This function is supposed to compare VMWare disks to physical disks. Never used so I don't know if it works or not.
.Example
Compare-DisksToVMDisks
This function is supposed to compare VMWare disks to physical disks. Never used so I don't know if it works or not.
#>
    ...
    Process
    {
        Try
        {
            ...
            if (($VmView = Get-View -ViewType VirtualMachine -Filter @{"Name" = $Vm}))
            {
                ...
                foreach ($VirtualSCSIController in ($VMView.Config.Hardware.Device | 
                    Where-Object {$_.DeviceInfo.Label -match "SCSI Controller"}))
                {
                        ...
                        Where-Object {$_.ControllerKey -eq $VirtualSCSIController.Key}))
                    {
                        ...
                        $virtualdisk.naa = $disks|
                            Where-Object {$_.name -like $VirtualDiskDevice.DeviceInfo.Label}|select -expand scsicanonicalname
                        # Match disks based on SCSI ID
                        $DiskMatch = $WinDisks | 
                            Where-Object { ($_.SCSIPort - 2 ) -eq $VirtualSCSIController.BusNumber -and $_.SCSITargetID -eq $VirtualDiskDevice.UnitNumber }
                        if ($DiskMatch)
                        {
                        ...
                        {
                            Write-Output "No matching Windows disk found for SCSI id $($VirtualDisk.SCSI_Id)"
                        }
                        $matchdisktopar = $disktopart|
                            Where-Object {$_.Antecedent -eq $diskmatch.__Path}
                        $matchlogtopart = $logtopart| 
                            Where-Object {$_.Antecedent -eq $matchdisktopar.Dependent}
                        $logicalmatch = $logical| 
                            Where-Object {$_.path.path -eq $matchlogtopart.dependent}
