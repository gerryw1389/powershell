...
Function Update-VMWareTools
{
    <#
.Synopsis
Connects to vSphere and updates the tools on VMs.
.Description
Connects to vSphere and updates the tools on VMs.
Requires snapin VMware.VimAutomation.Core
NOTE: This has not been tested yet.
.Example
Update-VMWareTools -Vcenter 'myserver.domain.com'
Connects to vSphere and updates the tools on VMs.
#>
    ...
    Process
    {
        Try
        {
            ...
            {
                Write-Log "Updating VM's in $folder"
                $virtualmachines = Get-Folder $folder | Get-VM | select-object -expandproperty Name
            }
            Else
            {
                Write-Log "Updating all VM's"
                $virtualmachines = Get-VM | Where-Object { $_.Extensiondata.Summary.Guest.ToolsVersionStatus -like 'guestToolsNeedUpgrade' } | 
                    select-object -Expandproperty Name
            }
 
            ForEach ($vm in $virtualmachines)
            {
                Write-Log "Updating VMware Tools on $VM"
