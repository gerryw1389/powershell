...
<#######<Script>#######>
<#######<Header>#######>
# Name: Get-VMNameToDatastoreInfo
<#######</Header>#######>
<#######<Body>#######>
Function Get-VMNameToDatastoreInfo
{
    <#
.Synopsis
This function will get the VMName and Datastore information from vCenter for all VMs and export it as a CSV.
.Description
This function will get the VMName and Datastore information from vCenter for all VMs and export it as a CSV.
Requires module VMware.PowerCLI
.Example
Get-VMNameToDatastoreInfo -Path c:\scripts\datasheet.csv
Sends a CSV to c:\scripts\datasheet.csv which contains VMName and Datastore for all VMs in vCenter.
.Note
For most information, you can just run: Get-VM | Export-CSV -Path $Path -NoTypeInformation
#>
    ...
    Begin
    {
        ...
        $VCServer = 'myserver.domain.com'
        $vcPassword = Read-Host 'Enter password' -AsSecureString
        $vccredential = New-Object System.Management.Automation.PSCredential ('domain\myuser', $vcPassword)
    ...
    Process
    {
        ...
        {
            $Vms = Get-Vm
            $row = [System.Collections.Generic.List[PSObject]]@()
            $Intro = 'VMName,Datastore'
            [void]$row.Add($Intro)
            foreach ($vm in $vms)
            {
                $n = $vm.name
                $list = $vm.datastoreidlist | out-string
                $list2 = $list.replace("`r`n", ",")
                [string]$together = "$n,$list2"
                [void]$row.Add($together)
            }
            $row | Export-CSV -Path $Path -NoTypeInformation -Force
        }
        Catch
        {
