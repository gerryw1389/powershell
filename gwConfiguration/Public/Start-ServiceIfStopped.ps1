...
Function Start-ServiceIfStopped
{
    <#
    .Synopsis
    Checks one or more services and, if they are stopped, starts them up.
    .Description
    Checks one or more services and, if they are stopped, starts them up.
    .Parameter Service
    The service name you want to pass. One or more can be passed at once. Best ran as a scheduled task.
    .Example
        ...
        Try
        {
            ForEach ($S in $Service)
            {
                If ($s.Status -eq "Stopped")
                {
                    ...
                    Write-Log "Service $S was stopped, started service"
                }
            }
        }
        Catch
