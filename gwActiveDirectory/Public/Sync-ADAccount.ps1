...
<#######</Header>#######>
<#######<Body>#######>
Function Sync-ADAccount
{
    <#
.Synopsis
Adds a certain group or user to an existing user's groups.
.Description
Adds a certain group or user to an existing user's groups.
.Example
Sync-ADAccount -Users 'gerryw', 'billyb' -AccountToAdd 'jimmyj'
Adds 'jimmyj' to all groups that 'gerryw' has and that 'billyb' has.
.Notes
Version History:
2018-09-06: Initial
#>
    ...
    Process
    {
        Try
        {
            foreach ($user in $users)
            {
                ...
                $groups = Get-ADPrincipalGroupMembership $user | Select-Object -Property Name
