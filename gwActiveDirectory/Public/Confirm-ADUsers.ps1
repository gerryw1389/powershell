<#######<Script>#######>
<#######<Header>#######>
# Name: Confirm-ADUsers
<#######</Header>#######>
<#######<Body>#######>
Function Confirm-ADUsers
{
    <#
.Synopsis
...
#>
    ...
    Begin
    {
        ...
        New-Item -ItemType File -Path "$PSScriptRoot\exist.txt" -Force | Out-Null
        New-Item -ItemType File -Path "$PSScriptRoot\dontexist.txt" -Force | Out-Null
        ...
        Try
        {
            ...
            foreach ($u in $users)
            {
                Try
                {
                    ...
                    Write-Output "User does exist: $u"
                    Write-output "Exists: $u" | Out-File -FilePath "$PSScriptRoot\exist.txt" -Encoding ascii -Append
                }
                Catch
                {
                    Write-Output "User doesn't exist: $u"
                    Write-output "Doesn't exist: $u" | Out-File -FilePath "$PSScriptRoot\dontexist.txt" -Encoding ascii -Append
                }
            }  
        }
        Catch
