<#######<Script>#######>
<#######<Header>#######>
# Name: Add-AccountsToGroupsInOU
<#######</Header>#######>
<#######<Body>#######>
Function Add-AccountsToGroupsInOU
{
    <#
.Synopsis
This function will add a group of AD Accounts to each group in a given OU.
.Description
This function will add a group of AD Accounts to each group in a given OU.
.Example
Add-AccountsToGroupsInOU -OU 'OU=Test,OU=Servers,DC=Domain,DC=com' -AccountsToAdd 'gerryw', 'tommyg'
This function will get all groups under 'OU=Test,OU=Servers,DC=Domain,DC=com' and add 'gerryw' and 'tommyg' to each one of them.
.Notes
Version History:
2018-12-06: Initial
#>
    ...
    Process
    {
        Try
        {
            ...
            {
                Write-Log "Adding accounts to Group: $Group"
                Foreach ($Account in $AccountsToAdd)
                {
                    Add-ADGroupMember -Identity $Group -Members $Account
                    Write-Log "Adding account: $Account for group: $group "
                }
            }
        }
        Catch
