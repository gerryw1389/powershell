...
Adds a member to a group.
.Example
Add-UserToGroup -Identity 'gerryw' -Group 'VPN_Users'
Adds a 'gerryw' to group 'VPN_Users'
.Notes
Version History:
2018-09-06: Initial
#>
    ...
    Process
    {
        Try
        {
            Try
            {
                $test = Get-ADUser -identity $Identity -ErrorAction stop
            }
            Catch
            {
                $test = $null
            }	
            If ( $($test.name.length) -gt 0)
            {
                Write-output "Adding $Identity to group: $Group"
                Add-ADGroupMember -Identity $Group -Members $Identity
            }
            else
            {
                write-output "$Identity does not exist"
            }
