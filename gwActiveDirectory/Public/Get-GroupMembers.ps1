...
Function Get-GroupMembers
{
    <#
.Synopsis
From a list of AD group names, this will extract out the members of those groups.
You might also try:
Import-Module Activedirectory
Get-ADGroupmember 'Domain Admins' -Recursive | Select-Object -Property Name
.Description
From a list of AD group names, this will extract out the members of those groups.
.Example
Get-GroupMembers -Groups 'Domain Admins', 'Server_Admins'
Shows a table of all groups contained in 'Domain Admins' and 'Server_Admins'
#>
    ...
    Process
    {
        Try
        {
            ...
            ForEach ($group in $Groups)
            {
                Try 
                {
                    $users = Get-ADGroupMember -Identity $group -ErrorAction stop
                    [void]$Array.Add("################")
                    [void]$Array.Add("Group: $group")
                    [void]$Array.Add($($users.samaccountname))
                    [void]$Array.Add("################")
                }
                Catch
                {
                    Write-Log "Group not found: $group"
                }
