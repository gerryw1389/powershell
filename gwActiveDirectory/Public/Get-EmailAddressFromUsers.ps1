<#######<Script>#######>
<#######<Header>#######>
# Name: Get-EmailAddressFromUsers
<#######</Header>#######>
<#######<Body>#######>
Function Get-EmailAddressFromUsers
{
    <#
.Synopsis
Returns email addresses (if set as the User Principle Name) from a list of AD SamAccountNames.
.Description
Returns email addresses (if set as the User Principle Name) from a list of AD SamAccountNames.ng!
.Example
Get-EmailAddressFromUsers -Users 'gerryw', 'billyb'
Will return the 'UserPrincipleName' attribute for user 'gerryw' and 'billyb'. 
The log will contain user to Email but output will just show emails.
#>
    ...
    Process
    {
        Try
        {
            ...
            {
                $CurrentUser = Get-ADUser -Identity $User
                [void]$EmailArray.Add($($CurrentUser.UserPrincipalName))
                Write-Log "User: $User has email: $($CurrentUser.UserPrincipalName)"
            }
            Write-Output $EmailArray
        }
        Catch
        {
