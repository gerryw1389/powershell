...
Function Get-Weather
{
    ...
    {   
        If ($City)
        {
            Write-ToString "Getting weather for $City"
            (curl http://wttr.in/$City -UserAgent "curl" ).Content
            # Cannot substitue "curl" for "Invoke-WebRequest -URI" ....
        }
        ElseIf ($JustToday)
        {
            Write-ToString "Getting weather for today's current location"
            (curl http://wttr.in/?0 -UserAgent "curl" ).Content
        }
        ElseIf ($TwoDays)
        {
            Write-ToString "Getting two days weather forcast for current location"
            (curl http://wttr.in/?2 -UserAgent "curl" ).Content
        }
        ElseIf ($ByZip)
        {
            Write-ToString "Getting weather for zipcode $ByZip"
            (curl http://wttr.in/$ByZip -UserAgent "curl" ).Content
        }
        ElseIf ($Moon)
        {
            Write-ToString "Getting today's moon phase"
            (curl http://wttr.in/moon -UserAgent "curl" ).Content
        }
        ElseIf ($MoonOnDate)
        {
            Write-ToString "Getting moon phase for $MoonOnDate"
            (curl http://wttr.in/moon@$MoonOnDate -UserAgent "curl" ).Content
        }
        Else
        {
            Write-ToString "Getting weather for current location"
